...
Map searchWords(String words) {
  List<Map> result = [];
  // normalize the search query: remove diacritics and normalize characters
  final needle = normalise(removeDiacritics(words.toString())).toLowerCase();

  for (var i in quran_text_normal) {
    final hay =
        normalise(removeDiacritics(i['content'].toString())).toLowerCase();
